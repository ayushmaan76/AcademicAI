...
# Lazy initialization for Sentence Transformer to avoid startup errors
...
def normalize_path(file_path):
    """Normalize file path to handle cross-platform path separators"""
    if file_path:
...
def data_ingestion():
    ...
    if uploaded_file:
        ...
        if existing_file:
            ...
            # Store normalized path in database
            normalized_path = normalize_path(file_path)
            c.execute("INSERT INTO files (name, path, size) VALUES (?, ?, ?)",
                      (uploaded_file.name, normalized_path, os.path.getsize(file_path)))
            conn.commit()
...
def delete_file(file_name):
    ...
    try:
        ...
        if file_path:
            # Normalize path
            normalized_path = normalize_path(file_path[0])
...
def clean_invalid_files():
    ...
    try:
        ...
        st.error(f"Error cleaning invalid files: {e}")
        return 0

def get_valid_documents():
    """Get list of documents that actually exist on disk"""
    try:
        c = conn.cursor()
        c.execute("SELECT name, path FROM files")
        ...
        for doc in all_documents:
            normalized_path = normalize_path(doc[1])
            if os.path.exists(normalized_path):
                valid_documents.append((doc[0], normalized_path))
            else:
                # Remove from database if file doesn't exist
                c.execute("DELETE FROM files WHERE name = ?", (doc[0],))
                conn.commit()
...
def summarizer():
    ...
    if st.button("Generate Summary"):
        ...
        try:
            ...
            st.info("The file may have been deleted. Please check 'Uploaded Files' and re-upload if necessary.")
            return
        except Exception as e:
...
def take_notes():
    ...
    try:
        # Fetch the list of valid documents
        documents = get_valid_documents()
...
def main():
    ...
    # Clean up invalid files from database on startup (only once per session)
